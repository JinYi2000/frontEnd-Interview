...
`Size`：此cookie大小
`http字段`：cookie的httponly属性。若为true，则只有在http请求头中会带有此cookie的信息，而不能通过document.cookie来访问此cookie（可以一定程度上防止信息盗取）。
...
4. 请求Root DNS，返回gTLD Server（国际顶尖域名服务器）给LDNS
LDNS请求gTLD，gTLD查找并返回域名对应的Name Server地址（网站注册的域名服务器）
...
3.	设置代理服务器转发。这种方式实际上还是同源。因此服务器之间的通信不存在跨域的问题，所以可以设置一个与客户端同源的代理服务器作为中间人。
# XSS和CSRF攻击
## XSS
